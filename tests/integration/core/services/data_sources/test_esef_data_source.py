...
XBRL_JSON_FIXTURES = get_fixture_path("data_sources", "xbrl_json")


class TestBusinessDescriptionFromXBRL(unittest.TestCase):
    """Test business description extraction from XBRL-JSON."""
    ...
    def test_extracts_kainos_business_description(self):
        text = self.source._extract_fact_by_concept(self.kainos_xbrl, self.source.BUSINESS_DESCRIPTION_TAG)
...
class TestRevenueFromXBRL(unittest.TestCase):
    """Test revenue extraction from XBRL-JSON."""
    ...
    def test_parses_revenue_correctly(self):
        ...
        assert revenue.source_tag == self.source.REVENUE_TAG
